...
module.exports.createProxyServer = function (opts) {
    ...
    if (opts['transSslPort']) {
        var transSslServer = net.createServer(function (socket) {
           ...
           socket.on('data', function (data) {
               buffers.push(data);
               var sni = sniparse.getSNI(data);
               if(sni.isHandshake && sni.needMoreData) {
				   console.log('Received partial SSL handshake, waiting for more data to process SNI');
                   /* wait for the next packet to finish processing */
               }
               else {
                   data = Buffer.concat(buffers);
                   socket.removeAllListeners('data');
                   var onEnabled = function (isEnabled) {
                       return sslProxy(isEnabled, sni.host, socket, data);
                   };
                   var onRejectSSL = function (reject) {
                       if (reject) {
                           socket.end();
                       } else {
                   ...
                   emitOrRun('shouldRejectSSL', function () { onRejectSSL(false); }, {hostName: sni.host, socket: socket}, onRejectSSL);
               }
           });
        });
