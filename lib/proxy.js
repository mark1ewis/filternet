...
module.exports.createProxyServer = function (opts) {
    ...
    httpServer.on('connect', function (request, socket, head) {
        ...
        //emitOrRun('shouldReject', function () { onReject(false); }, request, onReject);
        emitOrRun('shouldRejectSSL', function () { onRejectSSL(false); }, {hostName: request.headers.host, socket: socket}, onRejectSSL);
    });
    ...
    if (opts['transSslPort']) {
        var transSslServer = net.createServer(function (socket) {
           ...
           socket.on('data', function (data) {
               ...
               emitOrRun('shouldRejectSSL', function () { onRejectSSL(false); }, {hostName: hostName, socket: socket}, onRejectSSL);
           });
        });
