...
// Returns an SNI result:
// { isHandshake: BOOLEAN, host: STRING, needMoreData: BOOLEAN }
// 
// If the packet is not a handshake, isHandshake will be false.
// Else If the packet is a handshake but does not have enough data to
// determine SNI, then needMoreData will be true.
// Else host will be the value of the SNI host, or null if the SNI extension
// is not set.

var getSNI = function(buffer) {
	try {
		return getSNIUnsafe(buffer);
	}
	catch(e) {
		if(e instanceof RangeError) {
			return { isHandshake: true, needMoreData: true };
		}
		else throw e;
	}
}

var getSNIUnsafe = function (buffer) {
    ...
    while (currentPos < buffer.length) {
        if (buffer.readInt16BE(currentPos) === 0) {
            ...
            if (buffer[currentPos] != 0) {
    			...
    			return {isHandshake: true, needMoreData: false, host: null};
            }
            currentPos += 5;
            return { isHandshake: true, needMoreData: false, host: buffer.toString('utf8', currentPos, currentPos + sniLength - 5)};
        } else {
